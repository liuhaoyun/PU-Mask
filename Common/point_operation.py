...
def random_flip(batch_data, batch_gt):
    """ random flip a axis along x, y or z.
            Input:
              BxNx3 array, original batch of point clouds
            Return:
    ...
    p2 = 0.3333
    p3 = 1 - p1 - p2
    axis = np.random.choice([0,1,2],replace=False, p =[p1,p2,p3])
    batch_data[:,:,axis:axis+1] = -1.0 * batch_data[:,:,axis:axis+1]
    batch_gt[:,:,axis:axis+1] = -1.0 * batch_gt[:,:,axis:axis+1]
