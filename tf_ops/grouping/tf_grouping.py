...
if __name__=='__main__':
    ...
    np.random.seed(100)
    pts = np.random.random((32,20,64)).astype('float32')
    tmp1 = np.random.random((32,20,3)).astype('float32')
    tmp2 = np.random.random((32,8,3)).astype('float32')
    with tf.device('/gpu:0'):
        ...
        # value,index = tf.unique(idx)
    with tf.Session() as sess:
        ret2 = print(sess.run(idx))
        ret1 = print(sess.run(value))
